...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'infos') {
    ...
    header("Location: mon-profil.php?success=1");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'mdp') {
    ...
    if ($nouveau === $confirm && strlen($nouveau) >= 6) {
        ...
        $pdo->prepare("UPDATE utilisateur SET mot_de_passe = ? WHERE id_utilisateur = ?")
            ->execute([$hash, $id_admin]);
        header("Location: mon-profil.php?mdp=1");
        exit;
    } else {
        header("Location: mon-profil.php?mdp=error");
        exit;
    }
}
?>
...
<div class="layout">
  ...
  <aside class="sidebar">
    ...
    <nav class="nav">
      ...
      <a href="mon-profil.php"      class="nav-item active">👤 Mon Profil</a>
    </nav>
  ...
  <main class="main">
    ...
    <div class="pwd-success" style="margin-bottom:16px">✅ Informations sauvegardées.</div>
    <?php endif; ?>
            ...
            <div class="profil-info-grid">
              ...
              <div class="profil-info-item">
                ...
                <input type="date" class="form-input" name="date_naissance"
                       value="<?php echo $admin['date_de_naissance'] ?? ''; ?>">
              </div>
            </div>
        ...
        <div class="card" style="margin-top:16px">
          ...
          <form method="POST" action="mon-profil.php">
            ...
            <div class="profil-info-item">
              ...
              <input type="password" class="form-input" name="nouveau_mdp" placeholder="••••••••">
            </div>
            <div class="profil-info-item" style="margin-top:10px">
              <span class="profil-info-label">Confirmer</span>
              <input type="password" class="form-input" name="confirm_mdp" placeholder="••••••••">
            </div>
