...
<main class="main">
  ...
  <div class="card"><div class="card-title">Liste des enseignants <span class="badge-moyenne"><?= count($enseignants) ?> enseignant(s)</span></div>
    ...
    <?php foreach ($enseignants as $ens) : ?><tr><td><?= htmlspecialchars($ens['nom']) ?></td><td><?= htmlspecialchars($ens['prenom']) ?></td><td><?= htmlspecialchars($ens['email']) ?></td><td><?= htmlspecialchars($ens['telephone'] ?? '--') ?></td><td><?= htmlspecialchars($ens['specialite'] ?? '--') ?></td><td><?= htmlspecialchars($ens['cours'] ?? '--') ?></td>
        <td><a href="modifier-enseignant.php?id=<?php echo $ens["id_utilisateur"]; ?>">
                    <button class="btn-save">✏️ Modifier</button>
                  </a>



            <a href="enseignants.php?supprimer=<?= $ens['id_utilisateur'] ?>" onclick="return confirm('Supprimer cet enseignant ?')"><button class="btn-lock">🗑️ Supprimer</button></a></td></tr><?php endforeach; ?>
    </tbody></table>
  </div>
  <br><br>
