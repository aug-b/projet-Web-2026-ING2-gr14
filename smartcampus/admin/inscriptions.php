...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'accepter') {
    ...
    $stmt = $conn->prepare("SELECT * FROM utilisateur_en_attente WHERE id_attente = ?");
...
$stmt->bind_param(
    ...
    $role
        ...
        // Assigne le rôle
        if ($role === 'eleve') {
            $numero = 'E' . str_pad($id_user, 3, '0', STR_PAD_LEFT);
            $niveau = 'Seconde';
            $stmt = $conn->prepare("INSERT INTO eleve (id_utilisateur, numero_eleve, niveau_scolaire) VALUES (?, ?, ?)");
            $stmt->bind_param("iss", $id_user, $numero, $niveau);
            $stmt->execute();

        } elseif ($role === 'enseignant') {
            $specialite = '';
            $stmt = $conn->prepare("INSERT INTO enseignant (id_utilisateur, specialite) VALUES (?, ?)");
            $stmt->bind_param("is", $id_user, $specialite);
            $stmt->execute();

        } elseif ($role === 'admin') {
            $stmt = $conn->prepare("INSERT INTO admin (id_utilisateur) VALUES (?)");
            $stmt->bind_param("i", $id_user);
            $stmt->execute();
        }

        // Met à jour le statut
        $stmt = $conn->prepare("UPDATE utilisateur_en_attente SET statut = 'accepte' WHERE id_attente = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
    }
...
if (isset($_GET['refuser'])) {
    $id = $_GET['refuser'];
    $stmt = $conn->prepare("UPDATE utilisateur_en_attente SET statut = 'refuse' WHERE id_attente = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
...
$result   = $conn->query("SELECT * FROM utilisateur_en_attente WHERE statut = 'en_attente' ORDER BY date_demande DESC");
